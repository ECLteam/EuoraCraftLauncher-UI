<template>
  <div class="oms-root">
    <div class="mods-search-toolbar">
      ...
      <div v-if="instance" class="instance-context">
        <UiIcon name="game-controller" :size="14" />
        <span>{{ instance.displayName }}</span>
        <NTag size="small" :bordered="false">{{ instance.vanillaName }}</NTag>
        <NTag size="small" :bordered="false">{{ instance.primaryLoader }}</NTag>
      </div>
    </div>
