...
export const useVersionStore = defineStore('versions', () => {
  ...
  function startWatching(): void {
    ...
    stopWatching = versionInstallApi.onVersionsChanged(({ gamePath }) => {
  ...
  async function scanPath(path: string, force = false): Promise<ScannedVersion[]> {
    ...
    const request = versionInstallApi
      .scan([path], { force })
      .then((result) => {
  ...
  async function loadAll(force = false): Promise<void> {
    ...
    loadingCount.value += 1
    try {
      scannedVersions.value = await versionInstallApi.scan(paths, { force })
      const selected =
        versions.value.find(
  ...
  function removePath(path: string): void {
    ...
    versionInstallApi.invalidateScanCache(path)
