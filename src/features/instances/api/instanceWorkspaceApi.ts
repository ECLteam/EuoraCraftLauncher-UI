...
export const instanceWorkspaceApi = {
  folders: (target: InstanceTargetPayload, folder: 'instance' | 'mods' | 'saves' | 'screenshots' | 'logs' | 'crash-reports') =>
  ...
  worldBackups: (target: InstanceTargetPayload, worldId: string) =>
    call<Array<{ id: string; createdAt?: string; locked: boolean; automatic: boolean; size: number }>>('game_world_backup_list', { ...target, world_id: worldId }, '读取存档备份'),
  restoreWorldBackup: (target: InstanceTargetPayload, worldId: string, backupId: string) =>
    call<GameOperation>('game_world_backup_restore', { ...target, world_id: worldId, backup_id: backupId }, '恢复存档备份'),
  ...
  deleteWorldBackup: (target: InstanceTargetPayload, worldId: string, backupId: string) =>
    call<void>('game_world_backup_delete_to_trash', { ...target, world_id: worldId, backup_id: backupId }, '删除存档备份'),
  setWorldIcon: (target: InstanceTargetPayload, worldId: string, sourcePath: string) =>
    call<{ path: string }>('game_world_icon_set', { ...target, world_id: worldId, source_path: sourcePath }, '设置世界图标'),
  ...
  saveScreenshot: (target: InstanceTargetPayload, screenshotId: string, outputPath: string) =>
    call<{ path: string }>('game_screenshot_save_as', { ...target, screenshot_id: screenshotId, output_path: outputPath }, '另存截图'),
  ...
  saveServer: (target: InstanceTargetPayload, server: Partial<ServerEntry> & Pick<ServerEntry, 'name' | 'address'>) =>
    call<ServerEntry>('game_server_upsert', { ...target, server_id: server.id, name: server.name, address: server.address, favorite: server.favorite }, '保存服务器'),
  ...
  resources: (target: InstanceTargetPayload, resourceType: GameResourceType, worldId?: string) =>
    call<GameResource[]>('game_resource_list', { ...target, resource_type: resourceType, world_id: worldId }, '读取资源'),
  installResources: (target: InstanceTargetPayload, resourceType: GameResourceType, sourcePaths: string[], worldId?: string) =>
    call<GameOperation>('game_resource_install', { ...target, resource_type: resourceType, source_paths: sourcePaths, world_id: worldId }, '安装资源'),
  toggleResource: (target: InstanceTargetPayload, resourceType: GameResourceType, resourceId: string, enabled: boolean, worldId?: string) =>
    call('game_resource_toggle', { ...target, resource_type: resourceType, resource_id: resourceId, enabled, world_id: worldId }, '切换资源状态'),
  checkResourceUpdates: (target: InstanceTargetPayload, resourceType: GameResourceType, gameVersion: string, loader: string, worldId?: string) =>
    call<Array<Record<string, unknown>>>('game_resource_update_check', { ...target, resource_type: resourceType, game_version: gameVersion, loader, world_id: worldId }, '检查资源更新'),
  updateResource: (target: InstanceTargetPayload, resourceType: GameResourceType, resourceId: string, update: Record<string, unknown>, worldId?: string) =>
    call<GameOperation>('game_resource_update', { ...target, resource_type: resourceType, resource_id: resourceId, update, world_id: worldId }, '更新资源'),
  deleteResources: (target: InstanceTargetPayload, resourceType: GameResourceType, resourceIds: string[], worldId?: string) =>
    call<void>('game_resource_delete_to_trash', { ...target, resource_type: resourceType, resource_ids: resourceIds, world_id: worldId }, '删除资源'),
  exportResourceManifest: (target: InstanceTargetPayload, resourceType: GameResourceType, outputPath: string, outputFormat: 'json' | 'csv', worldId?: string) =>
    call<{ path: string }>('game_resource_manifest_export', { ...target, resource_type: resourceType, output_path: outputPath, output_format: outputFormat, world_id: worldId }, '导出资源清单'),
  ...
  repairFiles: (target: InstanceTargetPayload) => call<GameOperation>('game_instance_files_repair', target, '补全实例文件'),
